...
import { ensureClaudeDir, CLAUDE_DIR, PROJECTS_DIR } from '../core/config.js';
...
class SessionPicker {
   ...
   render(): void {
      ...
      const agentLabel = this.currentAgentFilter === 'all' ? '' : ` [${this.currentAgentFilter}]`;
      buf.push(`${ESC}[1;1H${CLEAR_LINE}${BOLD}  ${t('pickerTitle')}${RESET} ${DIM}(${this.filtered.length}/${this.allSessions.length})${agentLabel}${RESET}`);
      buf.push(`${ESC}[2;1H${CLEAR_LINE}${DIM}${'─'.repeat(Math.min(w, 100))}${RESET}`);
      ...
      for (let i = start; i < end; i++) {
         ...
         const prefixLen = 5 + 8 + s.dateStr.length + s.project.length + s.cnt.length + via.length;
         const maxSummary = Math.max(20, w - prefixLen - 4);
         const truncSummary = s.summary.length > maxSummary ? s.summary.slice(0, maxSummary - 1) + '…' : s.summary;

         if (isSelected) {
            buf.push(`${ESC}[${row};1H${CLEAR_LINE} ${INVERSE}${BOLD} ${label} ${num}. [${s.dateStr}] ${s.project}${s.cnt}  ${truncSummary}${via} ${RESET}`);
         } else {
            buf.push(`${ESC}[${row};1H${CLEAR_LINE}  ${color}${label}${RESET} ${DIM}${num}.${RESET} [${CYAN}${s.dateStr}${RESET}] ${BOLD}${s.project}${RESET}${DIM}${s.cnt}${RESET}  ${truncSummary}${DIM}${via}${RESET}`);
         }
      }
      ...
      buf.push(`${ESC}[${sepRow};1H${CLEAR_LINE}${DIM}${'─'.repeat(Math.min(w, 100))}${RESET}`);
      buf.push(`${ESC}[${sepRow + 1};1H${CLEAR_LINE}  ${DIM}↑↓${RESET} ${t('navigate')}  ${DIM}Tab${RESET} agent  ${DIM}Enter${RESET} ${t('open')}  ${DIM}^D${RESET} ${t('delete_')}  ${DIM}^A${RESET} ${t('aiSummary')}  ${DIM}^R${RESET} ${t('refresh')}  ${DIM}Esc${RESET} ${t('quit')}`);
      ...
      // Status bar
      if (this.statusText) {
         buf.push(`${ESC}[${sepRow + 3};1H${CLEAR_LINE}  ${DIM}${this.statusText}${RESET}`);
      } else {
         buf.push(`${ESC}[${sepRow + 3};1H${CLEAR_LINE}`);
      }

      buf.push(`${ESC}[${sepRow + 4};1H${CLEAR_LINE}  ${YELLOW}>${RESET} ${this.searchText}${SHOW_CURSOR}`);
...
export default async function picker(args: string[] = []): Promise<void> {
   ...
   if (cached && cached.length > 0 && !projectFilter && !searchPreFilter) {
      ...
      sessions = await loadSessions({ projectFilter, searchQuery: searchPreFilter, agentFilter: 'claude' });
   }

   if (sessions.length === 0) {
      // No cache, no Claude — try full load
      sessions = await loadSessions({ projectFilter, searchQuery: searchPreFilter });
   }
   ...
   loadSessions({ projectFilter, searchQuery: searchPreFilter, limit: 500 }).then((allSessions) => {
      const existingIds = new Set(p.allSessions.map((s) => s.id));
      const newSessions = allSessions.filter((s) => !existingIds.has(s.id));
      if (newSessions.length > 0) {
         p.allSessions = [...p.allSessions, ...newSessions].sort((a, b) => b.lastTs - a.lastTs);
